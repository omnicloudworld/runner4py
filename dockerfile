...
RUN \
    ...
    status_code=$(curl --write-out %{http_code} --silent --output /dev/null https://packages.cloud.google.com/apt/dists/gcsfuse-$release/main/binary-amd64/Packages.gz); \
    ...
    apt-key add -; \
    apt-get update; apt-get install -y gcsfuse sudo; \
    apt-get clean
